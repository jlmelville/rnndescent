...
expect_error(
  brute_force_knn(ui10, k = 4, metric = "not-a-real metric"),
  "metric"
)
...
expect_error(
  brute_force_knn_query(reference = ui4, query = ui6, k = 7),
  "k must be"
)
expect_error(
  brute_force_knn_query(reference = ui4, query = ui6, k = 0),
  "k must be"
)
expect_error(
...
test_that("brute force APIs reject invalid n_threads values", {
  expect_error(
    brute_force_knn(ui10, k = 4, n_threads = -1),
    "n_threads must be"
  )
  expect_error(
...
ui6_nnd <- brute_force_knn(lbit6, k = 4, metric = "hamming")
qnbrs4 <- brute_force_knn_query(
  reference = lbit6,
  query = lbit4,
  k = 4,
...
check_query_nbrs_idx(qnbrs4$idx, nref = nrow(lbit6))
expect_equal(sum(qnbrs4$dist), bit4q_hdsum)

ui4_nnd <- brute_force_knn(lbit4, k = 4, metric = "hamming")
qnbrs6 <- brute_force_knn_query(
  reference = lbit4,
  query = lbit6,
  k = 4,
...
check_nbrs(
  ...
  int6hd,
  tolerance = 1e-6,
  check_idx_order = FALSE,
  check_dist_order = TRUE
)
...
# sparse queries
expect_equal(
  brute_force_knn_query(ui10sp, ui10sp, k = 4),
  brute_force_knn(ui10sp, k = 4)
)
expect_equal(
  brute_force_knn_query(ui10sp6, ui10sp4, k = 4),
  brute_force_knn_query(ui10z6, ui10z4, k = 4),
  tolerance = 1e-6
)
expect_equal(
  brute_force_knn_query(ui10sp4, ui10sp6, k = 4),
  brute_force_knn_query(ui10z4, ui10z6, k = 4),
  tolerance = 1e-6
)
