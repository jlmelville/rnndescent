...
set.seed(1337); bit_rnn <- nnd_knn(bitdata, 4, use_cpp = FALSE,
...
# convergence.
set.seed(1337); bit_rnn <- nnd_knn(bitdata, 4, use_cpp = TRUE,
                                   rho = 0.9,
                                   metric = "hamming")
expect_equal(bit_rnn$idx, expected_hamm_idx, check.attributes = FALSE)
expect_equal(bit_rnn$dist, expected_hamm_dist, check.attributes = FALSE)

set.seed(1337); bit_rnn <- nnd_knn(bitdata, 4, use_cpp = TRUE, use_set = TRUE,
