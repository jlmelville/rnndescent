...
# multi-threading
set.seed(1337)
bit_rnn <- nnd_knn(bitdata, 4, metric = "bhamming", n_threads = 1)
expect_equal(bit_rnn$idx, expected_hamm_idx, check.attributes = FALSE)
expect_equal(bit_rnn$dist, expected_hamm_dist, check.attributes = FALSE)
...
context("NN descent bhamming queries")

set.seed(1337)
bit6_nnd <- nnd_knn(bit6, k = 4, metric = "bhamming")
qnbrs4 <- graph_knn_query(reference = bit6, reference_graph = bit6_nnd, query = bit4, k = 4, metric = "bhamming")
check_query_nbrs_idx(qnbrs4$idx, nref = nrow(bit6))
expect_equal(sum(qnbrs4$dist), bit4q_hdsum, tol = 1e-6)

set.seed(1337)
bit4_nnd <- nnd_knn(bit4, k = 4, metric = "bhamming")
qnbrs6 <- graph_knn_query(reference = bit4, reference_graph = bit4_nnd, query = bit6, k = 4, metric = "bhamming")
...
test_that("prepare search graph with hamming and explicit zero", {
  ...
  bit6_nnd$dist[6, 4] <- 0
  bit6_sgraph <-
    prepare_search_graph(bit6, bit6_nnd, metric = "bhamming", n_threads = 1)
  qnbrs4 <-
    graph_knn_query(
      reference = bit6,
