...
[@bratic2019influence]. In this vignette we will use synthetic data to explore
the problem, and demonstrate how k-occurrence can diagnose the existence of
a potential problem. We will also look at one straightforward (if expensive)
way to ameliorate the effects.
...
hist(pmin(g1000d_bfko, max(g2d_bfko)), main = "1000D 15-NN zoomed", 
     xlab = "k-occurrences")
```
...
hist(pmin(g1000d_nndko, max(g2d_bfko)), main = "1000D 15-NND zoomed", 
     xlab = "k-occurrences")
```
...
plot(g1000d_nndko, g1000d_bfko, xlab = "approximate", ylab = "exact", 
...
hist(pmin(g1000d_nndkoi1, max(g2d_bfko)), main = "1000D 15-NND (1 iter, zoomed)", 
     xlab = "k-occurrences")
```
...
in terms of run time. What about taking advantage of the stochastic nature of
the algorithm? If the results are sufficiently diverse between runs of NND, then
we could generate two graphs from two separate runs, and then merge the
the results.
...
g1000d_sgko <- as.vector(table(Matrix::t(g1000d_search_graph)@i + 1))
hist(g1000d_sgko, main = "search graph k-occurrences", xlab = "k-occurrences")
summary(g1000d_sgko)
sum(g1000d_sgko == 1)
```

By design, the pruned search graph is unable to exhibit the extreme hub behavior
that the neighbor graph displayed. There are still some anti-hubs, but around an
order of magnitude fewer.

With the search graph in hand, we can now search it using our original data
as a query:
...
but accuracies are now noticeably less improved.
