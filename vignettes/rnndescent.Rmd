...
**very** large, especially if you set `n_trees` to a large value.
...
step each time, so it's more efficient to prepare the index separately. Another
advantage of preparing the index as soon as possible is that the index will be
reduced in size, as the number of trees used for querying data is usually
substantially smaller than the number used for building the index.
