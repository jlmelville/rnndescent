...
approximate nearest neighbor methods to be a lot faster. We also need a function
...
```{r rnnd knn accuracy}
nn_accuracy(fmnist_train_rnnd, fmnist_train_bf)
```
```
0.9871422
```
...
```{r build hnsw index}
hnsw_index <- hnsw_build(uwot:::x2m(fmnist_train), verbose = TRUE, n_threads = 6)
```
...
```{r hnsw knn accuracy}
nn_accuracy(fmnist_train_hnsw, fmnist_train_bf)
```
```
[1] 0.9748644
```
...
nn_accuracy(fmnist_train_annoy, fmnist_train_bf)
```
```
[1] 0.9590367
```
...
```{r rnnd accuracy}
nn_accuracy(fmnist_test_rnnd, fmnist_test_bf)
```
```
[1] 0.95952
```
...
nn_accuracy(fmnist_test_hnsw, fmnist_test_bf)
```
```
[1] 0.9551133
```
...
nn_accuracy(fmnist_test_annoy, fmnist_test_bf)
```
```
[1] 0.9458733
```
