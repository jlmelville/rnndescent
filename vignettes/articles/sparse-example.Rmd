...
rows this is not a huge memory burden. But if you decided to use the origin
...
```{r L2 normalize}
tfidf_spl2 <-
  Matrix::Diagonal(x = 1 / sqrt(Matrix::rowSums(tfidf_sp^2))) %*% tfidf_sp
```
...
l1_normalize <- function(X) {
  ...
  Matrix::Diagonal(x = 1 / res) %*% X
}
tfidf_spl1 <- l1_normalize(tfidf_sp)
head(Matrix::rowSums(tfidf_spl1))
```
```
[1] 1 1 1 1 1 1
```
...
```{r dataset words}
mean_tfidf <- Matrix::colMeans(tfidf_sp)
...
```{r popular words}
mean_popular_tfidf <- Matrix::colMeans(tfidf_sp[popular_indices, ])
...
There's `love` right at the top. Not a lot of overlap with the average words.

```{r unpopular words}
mean_unpopular_tfidf <- Matrix::colMeans(tfidf_sp[unpopular_indices, ])
...
write about love. If you don't want to sound like Jane Austen, use "every" a
lot. And "Harriet".
